...
public class KakaoLoginService {
	...
	// 회원가입 여부 판단
	public Optional<User> findMember(String socialId, OauthType oauthType) {
		return userRepository.findBySocialIdAndOauthType(socialId, oauthType);
	}

	// 회원가입이 되어 있지 않은 경우 랜덤으로 닉네임을 생성하고 저장
	public User saveSocialMember(String socialId, String name, OauthType oauthType) {
		User user = User.builder()
			.name(name)
			.profileImage("https://firebasestorage.googleapis.com/v0/b/vitalroutes-467cb.appspot.com/o/profile.png?alt=media")
			.socialId(socialId)
			.oauthType(oauthType)
			.build();
	...
	public String getAccessToken(String code) {
		...
		// HTTP POST 요청
		ResponseEntity<KakaoTokenResponse> response = restTemplate.exchange(uriComponentsBuilder.toUri(), HttpMethod.GET, request, KakaoTokenResponse.class);
